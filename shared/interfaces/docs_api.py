...
OPENAPI_SPEC = {
    "openapi": "3.0.3",
    "info": {
        "title": "Vinevault Edge API",
        "version": "1.0.0",
        "description": "Edge API for CO2 telemetry ingestion and Vinevault device provisioning.",
    },
    ...
    "tags": [
        ...
        {"name": "Commands", "description": "Embedded device command delivery. Commands arrive via Kafka from the core system."},
    ...
    "components": {
        ...
        "schemas": {
            "CreateTelemetryRequest": {
                "type": "object",
                "required": ["deviceId", "timestamp", "uptime", "airQuality", "connectivity", "location", "healthStatus", "status"],
                "properties": {
            ...
            "AlertIncidentEvent": {
                "type": "object",
                "required": ["id", "alert_id", "device_id", "hardware_id", "metric", "threshold_metric", "threshold_value", "actual_value", "status", "occurred_at", "resolved_at"],
                "properties": {
                    "id": {"type": "integer", "description": "Edge-local event ID used for ACK."},
                    "alert_id": {"type": "string", "description": "Core incident identifier."},
                    "device_id": {"type": "string", "description": "Logical device identifier."},
                    "hardware_id": {"type": "string", "description": "Physical hardware identifier."},
                    "space_id": {"type": "string", "nullable": True, "description": "Optional location or grouping identifier."},
                    "metric": {"type": "string", "description": "Metric identifier (e.g., CO2, PM25, TEMPERATURE, HUMIDITY)."},
                    "threshold_metric": {"type": "string", "nullable": True, "description": "Configured threshold rule that triggered the incident."},
                    "threshold_value": {"type": "string", "nullable": True, "description": "Configured threshold value."},
                    "actual_value": {"type": "string", "nullable": True, "description": "Measured value that triggered the incident."},
                    "message": {"type": "string", "nullable": True, "description": "Human-readable incident message."},
                    "status": {"type": "string", "enum": ["ACTIVE", "RESOLVED"], "description": "ACTIVE opens or keeps the local alarm open; RESOLVED closes it."},
    ...
    "paths": {
        "/api/v1/device/telemetry": {
            "post": {
                ...
                "description": "Authenticates the device locally using the SQLite cache, validates sensor readings (CO2, temperature, humidity), and stores the optimized telemetry record with connectivity status.",
        ...
        "/api/v1/device/commands/{commandId}/ack": {
            "post": {
                ...
                "description": "Persists the embedded ACK locally and publishes it to Kafka for the core system.",
                "security": [{"DeviceCredentials": [], "DeviceApiKey": []}],
...
def swagger_docs():
    ...
    return render_template_string(
        ...
        <head>
          <meta charset="utf-8">
          <title>Vinevault Edge API Docs</title>
